...
describe HALog::Application do
    ...
    it "uses the :memory: database by default" do
       HALog::Application.new(%W[ --input-file #{testing_logfile} --report error]).run
       $stderr.string.size.should > 0
       $stdout.string.size.should > 0
    end        
    
    it "parses a file and runs a report" do
        HALog::Application.new(%W[ --database :memory: --input-file #{testing_logfile} --report error]).run
        $stderr.string.size.should > 0
        $stdout.string.size.should > 0
    end
    
    it "outputs the report to an output file if given" do
        HALog::Application.new(%W[ --database #{@tmp_db.path} --input-file #{testing_logfile} --report error --output-file #{@tmp_outfile.path}]).run
        $stderr.string.size.should > 0
        $stdout.string.size.should == 0
        @tmp_outfile.open
        @tmp_outfile.rewind
